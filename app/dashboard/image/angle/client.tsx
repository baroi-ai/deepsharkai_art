...
const RotationController = ({
  ...
  const handleMove = (clientX: number, clientY: number) => {
    ...
    let newPitch = startValues.current.pitch + deltaY * 0.005;
    ...
    setPitch(Number(newPitch.toFixed(2)));
  };
  ...
  return (
    <div className="w-full max-w-lg mx-auto mt-6 animate-in fade-in slide-in-from-bottom-4">
      <div className="flex flex-col items-center">
        ...
        >
          <div
            className="no-sidebar-swipe relative w-24 h-24 preserve-3d transition-transform duration-75 ease-linear will-change-transform"
            style={{
        ...
        <div className="no-sidebar-swipe flex items-center justify-center gap-2 mb-6 text-gray-500 text-[10px] uppercase tracking-widest">
          <Move3d className="w-3 h-3" />
          <span>Drag Cube</span>
        </div>

        {/* Sliders Area - 1 Row, 3 Columns */}
        <div className="no-sidebar-swipe w-full px-2 select-none grid grid-cols-3 gap-4 md:gap-8">
          {/* 1. Rotation (Yaw) */}
...
const ImageAngelChangerPage = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // ✅ Updated route to match the fixed backend route name
      const response = await fetch("/api/fal/angel", {
        method: "POST",
              ...
              {job.status === "processing" ? (
                ...
                job.urls.map((url, i) => (
                  <div key={i} className="relative group">
                    {/* ✅ Uses Next/Image with unoptimized=true to prevent COEP blocks */}
                    <Image
                      src={url}
                      alt="Rotated"
                      width={800}
